...
class _CleanerManagementScreenState
  ...
  Widget build(BuildContext context) {
    ...
    if (isMobile) {
      return AdminBaseLayout(
        ...
        child: _buildMobileLayout(cleanersAsync),
      );
    }

    return Scaffold(
      backgroundColor: AppTheme.modernBg,
      body: _buildDesktopLayout(cleanersAsync),
    );
  }
                  ...
                  data: (cleaners) {
                    ...
                    switch (_sortBy) {
                      ...
                      case 'tasks':
                        sortedCleaners.sort(
                          (a, b) =>
                              b.activeTaskCount.compareTo(a.activeTaskCount),
                        );
                        break;
                      case 'performance':
    ...
    return cleanersAsync.when(
      data: (cleaners) {
        ...
        switch (_sortBy) {
          ...
          case 'tasks':
            sortedCleaners.sort(
              (a, b) => b.activeTaskCount.compareTo(a.activeTaskCount),
            );
            break;
          case 'performance':
            // Sort by rating (calculated from active tasks)
  ...
  Widget _buildSummaryStats(List<CleanerProfile> cleaners) {
    ...
    final availableCleaners = cleaners
        .where((c) => c.activeTaskCount == 0)
        .length;
    final totalTasks = cleaners.fold<int>(
      0,
      (sum, c) => sum + c.activeTaskCount,
    );
              ...
              child: Row(
                children: [
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                      gradient: LinearGradient(
                        colors: cleaner.activeTaskCount == 0
                            ? [
                                AppTheme.success,
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 16),
                    _buildDetailSection('ID', cleaner.id, Icons.badge),
                    const SizedBox(height: 24),
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        onPressed: () {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text(
                                'Fitur detail performa sedang dalam pengembangan',
                              ),
  ...
  Widget _buildFAB() {
    return FloatingActionButton(
      onPressed: () {
        // Action untuk menambah petugas baru
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Fitur menambah petugas baru')),
        );
      },
